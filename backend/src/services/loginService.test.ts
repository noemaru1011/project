...
describe('LoginService', () => {
  ...
  describe('login', () => {
    ...
    it('存在しないメールアドレスの場合、InvalidCredentialsErrorを投げること', async () => {
      ...
      await expect(loginService.login('wrong@test.com', 'pass'))
    ...
    it('管理者のパスワードが間違っている場合、InvalidCredentialsErrorを投げること', async () => {
      ...
      await expect(loginService.login('admin@test.com', 'wrong_pass'))
        .rejects.toThrow(InvalidCredentialsError);
    });
    ...
    it('学生のパスワードが間違っている場合、InvalidCredentialsErrorを投げること', async () => {
      ...
      await expect(loginService.login('student@test.com', 'wrong_pass'))
        .rejects.toThrow(InvalidCredentialsError);
    });
  });
});
