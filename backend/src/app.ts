...
import { authMiddleware, requireRole, commonMiddlewares, errorMiddleware } from '@/middleware';
...
app.use(API_ROUTES.LOGOUT, authMiddleware, logoutRoutes);

//パスワードの変更は学生のみ
app.use(API_ROUTES.PASSWORD, authMiddleware, requireRole([ROLE.STUDENT]), passwordRoutes);
//学生・管理者両方使える
app.use(API_ROUTES.HISTORY_SEARCH, authMiddleware, historySearchRoutes);
app.use(API_ROUTES.HISTORY, authMiddleware, historyRoutes);
app.use(API_ROUTES.STUDENT_SEARCH, authMiddleware, studentSearchRoutes);
...
//管理者のみ
app.use(API_ROUTES.STUDENT, authMiddleware, requireRole([ROLE.ADMIN]), studentRoutes);
