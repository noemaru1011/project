...
describe('PasswordService', () => {
  ...
  describe('updatePassword', () => {
    ...
    it('正しい旧パスワードであれば、新パスワードをハッシュ化して更新すること', async () => {
      mockPasswordRepo.findByStudentId.mockResolvedValue({
        studentId: 's1',
    ...
    it('学生が存在しない場合、NoStudentErrorを投げること', async () => {
      ...
      await expect(passwordService.updatePassword(mockInput, 'unknown')).rejects.toThrow(
        NoStudentError,
      );
    });
    ...
    it('旧パスワードが一致しない場合、NotMatchPasswordErrorを投げること', async () => {
      ...
      await expect(passwordService.updatePassword(mockInput, 's1')).rejects.toThrow(
