...
describe('RadioGroup', () => {
  ...
  describe('Props テスト', () => {
    ...
    describe('error', () => {
      ...
      it('エラー時に fieldset が aria-invalid="true" になる', () => {
        render(
          <RadioGroup name="test" options={mockOptions} error="エラー" />
        );
    ...
    describe('column', () => {
      ...
      it('column=2 で2列グリッドになる', () => {
        const { container } = render(
          <RadioGroup name="test" options={mockOptions} column={2} />
        );
        const rows = container.querySelectorAll('.flex.flex-row');
      ...
      it('column=3 で3列グリッドになる', () => {
        const { container } = render(
          <RadioGroup name="test" options={mockOptions} column={3} />
        );
        const rows = container.querySelectorAll('.flex.flex-row');
      ...
      it('column=1 で縦一列になる', () => {
        const { container } = render(
          <RadioGroup name="test" options={mockOptions} column={1} />
        );
        const rows = container.querySelectorAll('.flex.flex-row');
      ...
      it('column=0 で縦一列になる（デフォルト動作）', () => {
        const { container } = render(
          <RadioGroup name="test" options={mockOptions} column={0} />
        );
        const rows = container.querySelectorAll('.flex.flex-row');
      ...
      it('column が options より多い場合、1行になる', () => {
        const { container } = render(
          <RadioGroup name="test" options={mockOptions} column={10} />
        );
        const rows = container.querySelectorAll('.flex.flex-row');
    ...
    describe('onChange', () => {
      ...
      it('onChange 未指定でもエラーにならない', async () => {
        ...
        await expect(
          user.click(screen.getByLabelText('オプション1'))
  ...
  describe('ユーザーインタラクション', () => {
    ...
    it('disabled の場合、クリックできない', async () => {
      ...
      const handleChange = vi.fn();
      render(
        <RadioGroup name="test" options={mockOptions} disabled onChange={handleChange} />
      );
  ...
  describe('スナップショット', () => {
    ...
    it('disabled の HTML 構造', () => {
      const { container } = render(
        <RadioGroup name="test" options={mockOptions} disabled />
      );
      expect(container.firstChild).toMatchSnapshot();
    });
