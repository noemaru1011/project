...
describe('Input component', () => {
  it('labelは表示されるか', () => {
    render(<Input id="labelTest" type="text" value="" onChange={() => {}} label="ラベル" />);
  ...
  it('labelは表示されないか', () => {
    ...
    expect(screen.queryByText('ラベルは存在しないのでnullを返します')).not.toBeInTheDocument();
  });

  it('初期値の制御はできるか', () => {
    render(<Input id="defaultValueTest" type="text" value="HelloWorld" onChange={() => {}} />);
  ...
  it('errorは表示されるか', () => {
    render(
      <Input id="errorTest" type="text" value="" onChange={() => {}} error="エラーがあります" />,
    );
    expect(screen.getByText('エラーがあります')).toBeInTheDocument();
  });

  it('typeが正しく反映されるか', () => {
    render(<Input id="typeTest" type="number" value="123" onChange={() => {}} />);
  ...
  it('disabledは反映されるか', () => {
    render(
      <Input id="disabledTest" type="text" value="非活性テスト" onChange={() => {}} disabled />,
    );
    const input = screen.getByRole('textbox');
  ...
  it('required時に必須マークは表示されるか', () => {
    render(
      <Input id="labelTest" type="text" value="" onChange={() => {}} required label="ラベル" />,
    );
    expect(screen.getByText('ラベル*')).toBeInTheDocument();
    expect(screen.getByLabelText('ラベル*')).toBeInTheDocument();
  });
